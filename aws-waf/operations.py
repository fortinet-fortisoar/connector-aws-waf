...
def _create_client(config):
    ...
    try:
        ...
        if config_type == "IAM Role":
            ...
            aws_access_key, aws_region, aws_secret_access_key, verify_ssl = _get_credentials_from_config(config)
            boto_client = client('wafv2', region_name=aws_region, aws_access_key_id=aws_access_key,
...
def build_payload(params):
    return {k: MAPPING.get(str(v), v) for k, v in params.items() if v is not None and v != ''}
...
def update_ip_set(config, params):
    ...
    AddressesToRemove = [] if not params.get('AddressesToRemove') else [str(x.strip()) for x in params.get('AddressesToRemove').split(",")]
    params.pop('AddressesToAdd', "")
    params.pop('AddressesToRemove', "")
    data = {
        'Name': params.get('Name'),
        'Scope': params.get('Scope'),
        'Id': params.get('Id')
    }
    response = get_ip_set(config, data)
    Addresses = response.get('IPSet').get('Addresses')
    if AddressesToAdd:
        Addresses = Addresses + AddressesToAdd
    if AddressesToRemove:
        Addresses = [item for item in Addresses if item not in AddressesToRemove]
    params['Addresses'] = Addresses
    Description = params.get('Description') if params.get('Description') else response.get('IPSet').get('Description')
    if Description:
